    ...
    public class DotExtractor
    {
        ...
        private void AddCharms()
        {
            ...
            var charmList = (from item in xml.Root.Elements("String")
                             ...
                             select new HotDot(int.Parse(id), "Charm", 0, "0", 0, 0, name, "", "",tooltip, iconName, "Buff", true, iconName)).ToList();
        ...
        private void RawExtract()
        {
            ...
            var redirects_to_ignore = new string[] {"64", "161", "182", "248", "252", "264"};
            var redirects_to_follow = new string[] { "64", "161", "182", "223", "248", "252", "264" };
            foreach (
                var file in
                    ...
                    Directory.EnumerateFiles(RootFolder + _region + "/StrSheet_Abnormality/"))
            {
                ...
                var Namedata = (from item in xml.Root.Elements("String")
                                ...
                                let tooltip = item.Attribute("tooltip") == null ? "" : SubValues(item.Attribute("tooltip").Value
                                    .Replace("$H_W_GOOD","").Replace("H_W_GOOD", "").Replace("$COLOR_END","").Replace("$H_W_BAD","").Replace("$H_W_Bad","").Replace("H_W_BAD","").Replace("$BR"," ").Replace("<br>", " ").Replace("\n"," ").Replace("\r", " "), abnormalid,subs)
