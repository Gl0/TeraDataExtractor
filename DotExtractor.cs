    ...
    public class DotExtractor
    {
        ...
        private void AddCharms()
        {
            ...
            var charmList = (from item in xml.Root.Elements("String")
                             ...
                             select new HotDot(int.Parse(id), "Charm", 0, "0", 0, 0, name, "", "",tooltip, iconName, "Buff", true)).ToList();
        ...
        private void RawExtract()
        {
            ...
            var Dots = "".Select(t => new { abnormalid = string.Empty, type = string.Empty, amount = string.Empty, method = string.Empty, time = string.Empty, tick = string.Empty, num = 0, property=string.Empty, isBuff=string.Empty }).ToList();
                    ...
                    Directory.EnumerateFiles(RootFolder + _region + "/Abnormality/"))
            {
                ...
                var Dotdata = (from item in xml.Root.Elements("Abnormal")
                               ...
                               where (((type == "51" || type == "52") && tick != "0") || (interesting.Contains(type) && (isShow != "False" || abnormalid=="201" || abnormalid == "202" || abnormalid== "10152050"))) && amount != "" && method != "" 
                               select new { abnormalid, type, amount, method, time, tick, num, property, isBuff }).ToList();                                  //// 201 202 - marked as not shown, but needed
                ...
                var parser = (from item in xml.Root.Elements("Abnormal")
                              ...
                              where (type == "223" || type == "248" || type == "264")
                ...
                Dotdata = (from item in xml.Root.Elements("Abnormal")
                               ...
                               select new { abnormalid, type, amount, method, time, tick, num, property,isBuff }).ToList();
                Dots = Dots.Union(Dotdata).ToList();
            ...
            var subs = (from dot in Dots
                        ...
                        let change = ((dot.method == "3" || dot.method == "4" || dot.method == "0") && dot.type!="227")
